  ...
  <div class="chatbox" @input="userHasInteracted = true;">
    ...
    <div class="top-panel" :disabled="isSending" :style="{ zIndex: 10 }">
      ...
      <div class="top-panel-right">
        <button title="download current conversation to text file" @click="downloadFile" class="clear-button"
                :disabled="isSending || emptyConversation">📥
        </button>
        <span style="width:4px"></span>
        <button title="delete ALL conversation histories" @click="clearHistories" class="clear-button"
                :disabled="isSending || !validID">💥
        </button>
        <span style="width:4px"></span>
        <button title="delete current conversation" @click="clearCurrentHistory" class="clear-button"
                :disabled="isSending || !validID || emptyConversation">❌
        </button>
      </div>
    </div>
    ...
    <Transition name="slide-down" :style="{ zIndex: 9}">
      <div class="top-slide-panel" v-show="showControlPanel">
        ...
        <div class="top-panel-left">
          ...
          <input title="input valid user id" class="idInput" v-model="user_id" placeholder="user id" @input="storeID"
                 :disabled="isSending" label="id"
                 :style="{ color: validID ? '#FF00FF' : '#FFFFFF'}"/>
        </div>
        <div class="top-panel-left" v-show="validID">
          <span style="width:auto; padding:5px;">🌡️temp:</span>
          <span title="current temperature (0 = consistency, 1 = creativity)"
                style="width:auto; padding:5px;">{{ parseFloat(temperature).toFixed(2) }}</span>
    ...
    <Transition name="slide-up">
      <div class="input-area" @input="userHasInteracted = true;" v-show="validID">
        <textarea title="type a message to interact with the ai"
                  ref="textarea" v-model="userMessage" placeholder="send a message" :disabled="isSending || !validID"
                  class="input"
      ...
      payload_schema: {
        ...
        "conversation_history": [
          ...
          {role: "human", name: "userID", content: "so what's this computational modeling biz?"},
          {
            ...
            temp: this.temperature,
            prompt: this.prompts_id,
            content: "it's like making a computer play pretend, but with cognitive processes."
          },
          {role: "human", name: "userID", content: "and how's that work?"},
          {
            ...
            temp: this.temperature,
            prompt: this.prompts_id,
            content: "you whip up a program that acts out the cognitive process you're curious about."
          },
